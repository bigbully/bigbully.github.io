...
我们间接引用对象的成员具有典型的面向对象的点符号，这是一个命名空间（即名称所指的对象）一个点号（周期性）其次是成员的名字。例如，调用ABS方法对mymodule对象我们会说mymodule ABS（42）。使用toString成员对象42，我们会说42.tostring。在一个对象中成员的实现可以称对方不合格（无对象名称前缀），但如果需要的话，他们使用一种特殊的名义封闭对象访问：这。
